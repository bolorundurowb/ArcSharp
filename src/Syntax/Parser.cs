...
public sealed class Parser
{
    ...
    private Token Advance() { var t = Current; if (_i < _tokens.Count - 1) _i++; return t; }
    ...
    public CompilationUnit ParseCompilationUnit()
    {
        ...
        {
            if (At(TokenKind.UsingKw)) { SkipUsing(); continue; }
            if (At(TokenKind.NamespaceKw)) { ParseNamespaceInto(cu); continue; }
            var t = ParseTypeDecl();
            if (t != null) cu.Types.Add(t);
            else { if (!At(TokenKind.EndOfFile)) Advance(); }
        }
        return cu;
    }
    ...
    private void ParseNamespaceInto(CompilationUnit cu)
    {
        ...
        while (!At(TokenKind.OpenBrace) && !At(TokenKind.Semicolon) && !At(TokenKind.EndOfFile)) Advance();
        if (Accept(TokenKind.Semicolon))
        {
            while (!At(TokenKind.EndOfFile))
            {
                if (At(TokenKind.UsingKw)) { SkipUsing(); continue; }
                var t = ParseTypeDecl();
                if (t != null) cu.Types.Add(t); else Advance();
            }
            return;
        }
    ...
    private TypeDecl? ParseTypeDecl()
    {
        ...
        while (true)
        {
            if (Accept(TokenKind.PublicKw) || Accept(TokenKind.PrivateKw) ||
                ...
                Accept(TokenKind.StaticKw)) continue;
            if (At(TokenKind.AbstractKw)) { isAbstract = true; Advance(); continue; }
            break;
        }

        if (At(TokenKind.ClassKw)) return ParseClass(isAbstract);
        if (At(TokenKind.StructKw)) return ParseStruct();
        if (At(TokenKind.InterfaceKw)) return ParseInterface();
    ...
    private void ParseMemberInto(TypeDecl owner, string typeName)
    {
        ...
        while (true)
        {
            if (Accept(TokenKind.PublicKw)) { isPublic = true; continue; }
            if (Accept(TokenKind.PrivateKw) || Accept(TokenKind.ProtectedKw) || Accept(TokenKind.InternalKw)) continue;
            if (Accept(TokenKind.StaticKw)) { isStatic = true; continue; }
            if (Accept(TokenKind.VirtualKw)) { isVirtual = true; continue; }
            if (Accept(TokenKind.OverrideKw)) { isOverride = true; continue; }
            if (Accept(TokenKind.AbstractKw)) { isAbstract = true; continue; }
            if (Accept(TokenKind.WeakKw)) { isWeak = true; continue; }
            if (Accept(TokenKind.NewKw)) continue;
            break;
        }

        if (At(TokenKind.Identifier) && Current.Text == typeName && Peek(1).Kind == TokenKind.OpenParen)
        {
            var ctor = new MethodDecl { Name = typeName, Line = line, IsConstructor = true, IsStatic = isStatic, IsPublic = isPublic };
            Advance(); // consume constructor name
            ParseParameterList(ctor.Parameters);
            if (Accept(TokenKind.Colon))
            {
                ctor.HasCtorInit = true;
                if (Accept(TokenKind.ThisKw)) ctor.CtorInitIsThis = true;
                else Expect(TokenKind.BaseKw);
        ...
        {
            var f = new FieldDecl { Type = type, Name = name, Line = line, IsStatic = isStatic, IsWeak = isWeak, IsPublic = isPublic };
            if (Accept(TokenKind.Assign)) ParseExpression();
    ...
    private Stmt ParseIf()
    {
        ...
        Stmt? els = null;
        if (Accept(TokenKind.ElseKw)) els = ParseStatement();
    ...
    private Stmt ParseFor()
    {
        ...
        Expect(TokenKind.OpenParen);
        Stmt? init = null;
        if (!At(TokenKind.Semicolon)) init = ParseSimpleStatement(requireSemicolon: false);
        Expect(TokenKind.Semicolon);
        Expr? cond = null;
        if (!At(TokenKind.Semicolon)) cond = ParseExpression();
        Expect(TokenKind.Semicolon);
        Expr? update = null;
        if (!At(TokenKind.CloseParen)) update = ParseExpression();
        Expect(TokenKind.CloseParen);
        var body = ParseStatement();
    ...
    private Expr ParseUnary()
    {
        ...
        if (At(TokenKind.OpenParen) && TryParseCast(out var cast)) return cast!;
        return ParsePostfix();
    }
    ...
    private bool TryParseCast(out Expr? cast)
    {
        ...
        if (!exprStart) { _i = save; return false; }
        var operand = ParseUnary();
    ...
    {
        string Eat(string n) { Advance(); return n; }
