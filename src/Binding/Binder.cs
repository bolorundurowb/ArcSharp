...
public sealed class Binder
{
    ...
    public BoundProgram Bind(CompilationUnit cu)
    {
        ...
        {
            if (_types.ContainsKey(td.Name)) { Report(td.Line, $"duplicate type '{td.Name}'"); continue; }
            var sym = new TypeSymbol
            {
                Name = td.Name,
                Kind = td switch { ClassDecl => TypeKind.Class, StructDecl => TypeKind.Struct, _ => TypeKind.Interface }
            };
            if (td is ClassDecl cd) sym.ClassSyntax = cd;
            else if (td is StructDecl sd) sym.StructSyntax = sd;
            else if (td is InterfaceDecl id) sym.InterfaceSyntax = id;
            _types[td.Name] = sym;
        }
        ...
        foreach (var td in cu.Types)
        {
            if (!_types.TryGetValue(td.Name, out var sym)) continue;
            if (td is ClassDecl cd)
            {
                foreach (var bn in cd.InterfaceNames)
                {
                    if (!_types.TryGetValue(bn, out var b)) { Report(td.Line, $"unknown base type '{bn}'"); continue; }
                    if (b.Kind == TypeKind.Class)
                    {
                        if (sym.BaseType != null) Report(td.Line, $"multiple base classes for '{td.Name}'");
                        sym.BaseType = b;
                    }
                    else if (b.Kind == TypeKind.Interface) sym.Interfaces.Add(b);
                    else Report(td.Line, $"'{bn}' cannot be a base of a class");
                }
            }
        }
        ...
        program.Types.AddRange(_types.Values.Where(t => t.Kind is TypeKind.Class or TypeKind.Struct or TypeKind.Interface));
        foreach (var t in program.Types)
        {
            foreach (var m in t.Methods)
            {
                if (m.Syntax?.Body == null) continue;
    ...
    private void LayoutType(TypeSymbol t)
    {
        ...
        foreach (var fd in decl.Members.OfType<FieldDecl>())
        {
            ...
            var fs = new FieldSymbol { Name = fd.Name, Type = ft, Owner = t, IsStatic = fd.IsStatic, IsWeak = fd.IsWeak };
            if (fd.IsWeak && !ft.IsReferenceType) Report(fd.Line, "'weak' may only be applied to reference-typed fields");
            if (fs.IsStatic) t.StaticFields.Add(fs);
            else { fs.Index = t.InstanceFields.Count; t.InstanceFields.Add(fs); }
        }
        ...
        foreach (var md in decl.Members.OfType<MethodDecl>())
        {
            ...
            if (m.IsOverride)
            {
                ...
                if (baseM != null) { m.VtableSlot = baseM.VtableSlot; t.Vtable[m.VtableSlot] = m; }
                else { m.VtableSlot = t.Vtable.Count; t.Vtable.Add(m); }
            }
            else if (m.IsVirtual)
            {
    ...
    private IEnumerable<TypeSymbol> AllInterfaces(TypeSymbol t)
    {
        ...
        while (cur != null)
        {
            foreach (var i in cur.Interfaces) if (seen.Add(i)) yield return i;
            cur = cur.BaseType;
        }
    }
    ...
    private MethodSymbol? FindMethod(TypeSymbol? t, string name, int argc)
    {
        ...
        {
            foreach (var m in t.Methods)
                if (!m.IsConstructor && m.Name == name && m.Parameters.Count == argc) return m;
            t = t.BaseType;
        }
        return null;
    }
    ...
    {
        foreach (var m in t.Methods) if (m.IsConstructor && m.Parameters.Count == argc) return m;
        return null;
    }
    ...
    private BoundMethodBody BindMethodBody(TypeSymbol type, MethodSymbol m)
    {
        ...
        if (m.IsConstructor)
        {
            ...
            if (initCall != null) block.Statements.Insert(0, new BoundExprStmt { Expression = initCall });
        }
    ...
    private BoundExpr? BuildCtorInit(MethodSymbol ctor)
    {
        ...
        if (targetType == null) { Report(md.Line, "'base' constructor call has no base class"); return null; }
    ...
    private BoundBlock BindBlock(BlockStmt b)
    {
        ...
        foreach (var s in b.Statements) bb.Statements.Add(BindStatement(s));
        PopScope();
        return bb;
    }
    ...
    private BoundStmt BindStatement(Stmt s) => s switch
    {
        ...
        IfStmt i => new BoundIf { Condition = BindToBool(i.Condition), Then = BindStatement(i.Then), Else = i.Else == null ? null : BindStatement(i.Else) },
    ...
    private BoundStmt BindLocalDecl(LocalDeclStmt d)
    {
        ...
        if (d.Type.Name == "var")
        {
            if (init == null) { Report(d.Line, "'var' requires an initializer"); type = _error; }
            else type = init.Type == _null ? _error : init.Type;
        }
        else type = ResolveType(d.Type);

        if (init != null) init = Convert(init, type, d.Line);
    ...
    {
        if (r.Value == null) return new BoundReturn { Value = null };
    ...
    {
        if (_curMethod.IsStatic) Report(0, "'this' is not available in a static method");
    ...
    private BoundExpr BindName(NameExpr n)
    {
        ...
        if (local != null) return new BoundLocal { Symbol = local, Type = local.Type };
        if (_params.TryGetValue(n.Name, out var p)) return new BoundParam { Symbol = p, Type = p.Type };
    ...
    private BoundExpr BindMemberAccess(MemberAccessExpr m)
    {
        ...
        if (f != null) return new BoundFieldAccess { Receiver = recv, Field = f, Type = f.Type };
    ...
    private BoundExpr BindInvocation(InvocationExpr inv)
    {
        ...
        if (inv.Callee is MemberAccessExpr ma)
        {
            ...
            if (recv.Type.Kind == TypeKind.Interface)
            {
                ...
                if (_selectors.TryGetValue(key, out var sel))
                {
                    ...
                    if (call is BoundCall bc) { bc.Interface = true; bc.Virtual = false; bc.Selector = sel; }
                    return call;
                }
            ...
            if (method == null) { Report(inv.Line, $"'{recv.Type}' has no method '{ma.Name}'"); return Err(); }
            var c = MakeCall(method.IsStatic ? null : recv, method, args, inv.Line);
            if (c is BoundCall b2) b2.Virtual = method.IsVirtual || method.IsOverride;
            return c;
        }
        ...
        if (inv.Callee is NameExpr nm)
        {
            ...
            if (method == null) { Report(inv.Line, $"unknown method '{nm.Name}'"); return Err(); }
    ...
    private MethodSymbol? FindInterfaceMethod(TypeSymbol iface, string name, int argc)
    {
        foreach (var m in iface.Methods)
            if (m.Name == name && m.Parameters.Count == argc) return m;
        return null;
    }
    ...
    private BoundExpr BindConsole(string name, List<BoundExpr> args, int line)
    {
        ...
        { Report(line, $"Console has no method '{name}'"); return Err(); }
        if (args.Count == 0) return new BoundConsoleCall { Which = which, Argument = null, Type = _void };
        if (args.Count != 1) { Report(line, "Console.Write/WriteLine takes 0 or 1 argument"); }
    ...
    private BoundExpr BindIndex(IndexExpr ix)
    {
        ...
        { Report(ix.Line, $"cannot index '{recv.Type}'"); return Err(); }
    ...
    private BoundExpr BindBinary(BinaryExpr b)
    {
        ...
        if (l.Type == _error || r.Type == _error) return Err();

        // logical
        if (b.Op is TokenKind.AmpAmp or TokenKind.PipePipe)
        {
            if (l.Type != _bool || r.Type != _bool) Report(b.Line, "logical operator requires 'bool' operands");
    ...
    private BoundExpr BindUnary(UnaryExpr u)
    {
        ...
        {
            if (o.Type != _bool) Report(u.Line, "'!' requires a 'bool' operand");
    ...
    private BoundExpr BindAssign(AssignExpr a)
    {
        ...
        { Report(a.Line, "the left-hand side of an assignment must be a variable, field, or element"); return Err(); }
    ...
    {
        if (e.Type == target || target == _error || e.Type == _error) return Setnull(e, target);
        // null literal -> any reference type
        if (e.Type == _null && target.IsReferenceType) return new BoundConversion { Operand = e, Type = target };
        // int -> long
    ...
    private bool RefCompatible(TypeSymbol from, TypeSymbol to)
    {
        ...
        if (from.IsSubclassOf(to) || to.IsSubclassOf(from)) return true;
        if (to.Kind == TypeKind.Interface && from.Implements(to)) return true;
        if (from.Kind == TypeKind.Interface) return true; // downcast from interface, checked at runtime (unchecked here)
        if (from == _string && to == _string) return true;
        return false;
    }
