...
public sealed class Emitter(BoundProgram program, string triple, bool boundsChecks = true)
{
    ...
    private string Inst(string rhs) { EnsureBlock(); var t = T(); _body.Add($"  {t} = {rhs}"); return t; }
    private void Do(string s) { EnsureBlock(); _body.Add($"  {s}"); }
    private void Term(string s) { EnsureBlock(); _body.Add($"  {s}"); _terminated = true; }
    private void Lbl(string l) { _body.Add($"{l}:"); _terminated = false; }
    private void EnsureBlock() { if (_terminated) { var l = L("dead"); _body.Add($"{l}:"); _terminated = false; } }
    ...
    public string Emit()
    {
        ...
        foreach (var s in _strings) _mod.AppendLine(s);
    ...
    private void EmitTypeInfo(TypeSymbol t, StringBuilder fns)
    {
        ...
        {
            if (!f.Type.IsReferenceType) continue;
    ...
    private static string FnPtrType(MethodSymbol m)
    {
        ...
        foreach (var p in m.Parameters) ps.Add(p.Type.LlvmType);
    ...
    private void EmitMethod(BoundMethodBody b, StringBuilder fns)
    {
        ...
        if (!m.IsStatic) ps.Add("i8* %this");
        foreach (var p in m.Parameters) ps.Add($"{p.Type.LlvmType} %a{p.Index}");
        ...
        if (!isVoid)
        {
            ...
            if (m.ReturnType.IsReferenceType) _entry.Add("  store i8* null, i8** %retval");
        }
        ...
        if (!_terminated)
        {
            if (!isVoid) StoreRet(DefaultVal(m.ReturnType), m.ReturnType);
        }
        ...
        fns.AppendLine("entry:");
        foreach (var l in _entry) fns.AppendLine(l);
        foreach (var l in _body) fns.AppendLine(l);
        fns.AppendLine("}");
        fns.AppendLine();
    }
    ...
    private void EmitStmt(BoundStmt s)
    {
        switch (s)
        {
            case BoundBlock b: foreach (var x in b.Statements) EmitStmt(x); break;
    ...
    private void EmitIf(BoundIf s)
    {
        ...
        Lbl(thenL);
        EmitStmt(s.Then);
        if (!_terminated) Term($"br label %{endL}");
        if (s.Else != null)
        {
            Lbl(elseL);
            EmitStmt(s.Else);
    ...
    private void EmitWhile(BoundWhile s)
    {
        ...
        Lbl(bodyL);
        EmitStmt(s.Body);
        if (!_terminated) Term($"br label %{condL}");
        Lbl(endL);
    }

    private void EmitFor(BoundFor s)
    {
        if (s.Init != null) EmitStmt(s.Init);
        ...
        else Term($"br label %{bodyL}");
        Lbl(bodyL);
        EmitStmt(s.Body);
        if (!_terminated) Term($"br label %{stepL}");
        Lbl(stepL);
        if (s.Update != null)
        {
            var mark = _stmtTemps.Count;
    ...
    private string EmitR(BoundExpr e)
    {
        switch (e)
        {
            ...
            case BoundThis: { Do("call void @arc_retain(i8* %this)"); return Inst("load i8*, i8** %this.addr"); }
    ...
    private string EmitLoadSlot(string slot, TypeSymbol type)
    {
        ...
        if (type.IsReferenceType) Do($"call void @arc_retain(i8* {v})");
        return v;
    }
    ...
    private string EmitFieldRead(BoundFieldAccess fa)
    {
        ...
        if (fa.Field.Type.IsReferenceType) Do($"call void @arc_retain(i8* {val})");
        return val;
    }
    ...
    {
        string? recv = null;
        if (c.Receiver != null) { recv = EmitR(c.Receiver); _stmtTemps.Add(recv); }
    ...
    private string ElemAddr(string arr, string idx, TypeSymbol elem)
    {
        ...
        if (_boundsChecks) Do($"call void @arc_bounds_check(i8* {arr}, i64 {i64})");
    ...
    private string EmitIndexRead(BoundIndex ix)
    {
        ...
        if (ix.Type.IsReferenceType) Do($"call void @arc_retain(i8* {v})");
        return v;
    }
    ...
    private string EmitLogic(BoundBinary b)
    {
        ...
        if (b.Op == TokenKind.AmpAmp) Term($"br i1 {l}, label %{rhsL}, label %{endL}");
        else Term($"br i1 {l}, label %{endL}, label %{rhsL}");
        Lbl(rhsL);
        var r = EmitR(b.Right);
    ...
    private string EmitAssign(BoundAssign a)
    {
        ...
        {
            case BoundLocal lo:
                if (lo.Type.IsReferenceType) { Do($"call void @arc_assign_take(i8** {_localSlot[lo.Symbol.Id]}, i8* {value})"); Do($"call void @arc_retain(i8* {value})"); }
                else Do($"store {lo.Type.LlvmType} {value}, {lo.Type.LlvmType}* {_localSlot[lo.Symbol.Id]}");
                return value;
            case BoundParam p:
    ...
    private void EmitMain(StringBuilder fns)
    {
        ...
        fns.AppendLine("entry:");
        if (entry == null) { fns.AppendLine("  ret i32 0"); fns.AppendLine("}"); return; }
        ...
        else
        {
            ...
            fns.AppendLine("  call void @arc_report()");
            if (entry.ReturnType.LlvmType == "i32") fns.AppendLine("  ret i32 %r");
            else { fns.AppendLine("  %rc = trunc i64 %r to i32"); fns.AppendLine("  ret i32 %rc"); }
        }
        fns.AppendLine("}");
    }
}
